...
export const useEditsStore = defineStore('edits', {
  ...
  actions: {
    ...
    handleServerFrame(frame: ServerFrame): void {
      ...
      switch (event.type) {
        case 'staged_edit_created': {
          this.ensureList(conversationId);
          ...
          const conversation = useConversationsStore().conversations.find((c) => c.id === conversationId);
    ...
    ensureList(conversationId: string): StagedEditDto[] {
      const existing = this.byConversation[conversationId];
      if (existing) return existing;
      const created: StagedEditDto[] = [];
      this.byConversation[conversationId] = created;
      return created;
    },
  },
});
