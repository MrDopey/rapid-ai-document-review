...
- **FR-044**: System MUST listen only on the local loopback interface by default, and MUST NOT require or provide user authentication in v1. Loopback binding is the feature's default and documented access-control model. An operator MAY explicitly configure a different bind host (e.g., to reach the application from a containerized development environment via port-forwarding) as a deliberate, visible opt-in; this MUST NOT happen silently, and MUST NOT change the default itself — an unconfigured deployment MUST still bind to loopback only. When a non-default host is explicitly configured, the system MUST emit a clearly visible, logged warning identifying that it is bound beyond loopback with no authentication in place. Exposing the application beyond the machine it runs on remains an unsupported configuration unless the operator supplies an equivalent access-control mechanism of their own — this feature provides none.
...
- There is a single local user/account in v1; no multi-user authentication, authorization, or sharing is required. Localhost-only access is the enforced default (FR-044); an operator may explicitly opt into a wider bind host (e.g., for a containerized development environment), but the application still provides no authentication of its own, so the application is not designed to be safely exposed to other devices on a network or the internet unless the operator supplies an equivalent access-control mechanism themselves.
...
- The localhost-only, no-authentication posture is not merely assumed but enforced in code as the feature's default access-control model (FR-044); an explicit operator opt-in to a wider bind host is a deliberate, logged deviation from that default, not a change to it — the application still authenticates no one.
